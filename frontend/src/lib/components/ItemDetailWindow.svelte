...
<Draggable {left} {top}>
	<div class="window">
		...
		{#if isSpecialist(item.data)}
			...
			<p class="light-orange">
				{formatString(get_const_string(ConstStringKey.UpgradingGrade), item.data.upgrade_level)}<br
				/>
				{formatString(
			...
			<p class="light-orange">
				{formatString(get_const_string(ConstStringKey.RemainingPoints), item.data.remaining_points)}
			</p>
		{/if}
